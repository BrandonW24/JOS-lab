 ...
 * Exercise 1. In the file kern/pmap.c, you must implement code for the following functions
	...
	boot_alloc()
	mem_init() (only up to the call to check_page_free_list(1))6
	page_init()
	page_alloc()
	page_free()
 * 
...
mem_init(void)
{
	...
	we can now map memory using boot_map_region
	...
	*/
	...
	// Your code goes here:
	/*
	KERNBASE, PA range : [0, 2^32 - KERNBASE),	kernel RW, user NONE
	*/
...
page_alloc(int alloc_flags)
{
	...
	//Run a check to see if page_free_list is valid first
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	//This might not be working as I think it does
	// Fill this function in
	//pgdir = page directory, pgdir_walk should return a pointer to the page table entry PTE 
	//for linear address 'va' 
	/*
	This function was helpful to look at :
	...
	*/
	/*
	int page_direc = PDX(va);
	int index      = PTX(va);
	*/
	//Next we check to see if
	// The relevant page table page does not exist.
	/* This if statement looks similar to what is being described
		if (!(*pgdir & PTE_P))
			return ~0;
	*/


//This does not appear to work but I'm keeping it around.
/*
	if(!(pgdir[page_direc] & PTE_P)){
		// If this is true, and create == false, then pgdir_walk returns NULL.
		if(create){
			//Allocate the zeroth page and have a safety return NULL if the allocation
			//falls through.
			...
			pag->pp_ref++;
			pgdir[page_direc] = page2pa(pag) | PTE_P | PTE_U | PTE_W;
		} else {
			return NULL;
		}
	}

	pte_t *pg_point = KADDR(PTE_ADDR(pgdir[page_direc]));
	//int PTE_ind = pg_point + index;
	return pg_point + index;
	*/
